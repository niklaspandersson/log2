...
export class FetchService {
  private _authService: IAuthService|undefined = undefined;
  public get authService(): IAuthService|undefined { return this._authService; }
  public set authService(auth: IAuthService|undefined) { this._authService = auth; }
  ...
  protected async doFetch(url: string, options?: Partial<RequestInit>) {
    ...
    if (!res.ok) {
      if (res.status === 401 && this.authService?.accessToken) {
...
export default class FetchDataService<T> extends FetchService {
  ...
  async getAll(params?:URLSearchParams) {
    return this.doFetch(this.url + (params ? `?${params.toString()}` : ""), this.createOptions("GET")) as Promise<T[]>;
  }
  ...
  async update(id: string, data: Partial<T>) {
    return this.doFetch(`${this.url}/${id}`, this.createOptions("PATCH", data));
  }
